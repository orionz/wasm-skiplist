...
describe('SkipList', () => {
  describe('.indexOf()', () => {
    it('should return -1 on an empty list', async () => {
      const {SkipList} = await lib;
      let s = new SkipList()
      assert.strictEqual(s.indexOf('hello'), -1);
    })

    it('should return -1 for a nonexistent key', async () => {
      const {SkipList} = await lib;
      let s = new SkipList().insertAfter(null, 'a', 'a')
      assert.strictEqual(s.indexOf('b'), -1)
    })

    it('should return 0 for the first list element', async () => {
      const {SkipList} = await lib;
      let s = new SkipList().insertAfter(null, 'b', 'b').insertAfter('b', 'c', 'c').insertAfter(null, 'a', 'a')
      assert.strictEqual(s.indexOf('a'), 0)
    })

    it('should return length-1 for the last list element', async () => {
      const {SkipList} = await lib;
    ...
    it('should adjust based on removed elements', async () => {
      const {SkipList} = await lib;
  ...
  describe('.length', () => {
    it('should be 0 for an empty list', async () => {
      const {SkipList} = await lib;
      let s = new SkipList()
      assert.strictEqual(s.length, 0)
    })

    it('should increase by 1 for every insertion', async () => {
      const {SkipList} = await lib;
      let s = new SkipList().insertAfter(null, 'a', 'a').insertAfter('a', 'b', 'b').insertAfter('b', 'c', 'c')
      assert.strictEqual(s.length, 3)
    })

    it('should decrease by 1 for every removal', async () => {
      const {SkipList} = await lib;
      let s = new SkipList().insertAfter(null, 'a', 'a').insertAfter('a', 'b', 'b').insertAfter('b', 'c', 'c')
      assert.strictEqual(s.length, 3)
      s = s.removeKey('b')
  ...
  describe('.keyOf()', () => {
    it('should return null on an empty list', async () => {
      const {SkipList} = await lib;
      let s = new SkipList()
      assert.strictEqual(s.keyOf(0), null)
    })

    it('should return null for an index past the end of the list', async () => {
      const {SkipList} = await lib;
      let s = new SkipList().insertAfter(null, 'a', 'a').insertAfter('a', 'b', 'b')
      assert.strictEqual(s.keyOf(2), null)
    })

    it('should return the first key for index 0', async () => {
      const {SkipList} = await lib;
      let s = new SkipList().insertAfter(null, 'a', 'a').insertAfter('a', 'b', 'b').insertAfter('b', 'c', 'c')
      assert.strictEqual(s.keyOf(0), 'a')
    })

    it('should return the last key for index -1', async () => {
      const {SkipList} = await lib;
      let s = new SkipList().insertAfter(null, 'a', 'a').insertAfter('a', 'b', 'b').insertAfter('b', 'c', 'c')
      assert.strictEqual(s.keyOf(-1), 'c')
    })

    it('should return the last key for index length-1', async () => {
      const {SkipList} = await lib;
      let s = new SkipList().insertAfter(null, 'a', 'a').insertAfter('a', 'b', 'b').insertAfter('b', 'c', 'c')
      assert.strictEqual(s.keyOf(2), 'c')
    })

    it('should not count removed elements', async () => {
      const {SkipList} = await lib;
      let s = new SkipList().insertAfter(null, 'a', 'a').insertAfter('a', 'b', 'b').insertAfter('b', 'c', 'c')
      s = s.removeKey('b')
      assert.strictEqual(s.keyOf(0), 'a')
  ...
  describe('.getValue()', () => {
    it('should return undefined for a nonexistent key', async () => {
      const {SkipList} = await lib;
      let s = new SkipList()
      assert.strictEqual(s.getValue('foo'), undefined)
    })

    it('should return the inserted value when present', async () => {
      const {SkipList} = await lib;
  ...
  describe('.setValue()', () => {
    it('should throw an exception when setting a nonexistent key', async () => {
      const {SkipList} = await lib;
...
//      assert.strictEqual(s.getValue('key2'), 'value2')  // FIXME - moved values - bla bla
  ...
  describe('.insertIndex()', () => {
    it('should insert the new key-value pair at the given index', async () => {
      const {SkipList} = await lib;
    ...
    it('should insert at the head if the index is zero', async () => {
      const {SkipList} = await lib;
  ...
  describe('.removeIndex()', () => {
    it('should remove the value at the given index', async () => {
      const {SkipList} = await lib;
    ...
    it('should raise an error if the given index is out of bounds', async () => {
      const {SkipList} = await lib;
      let s = new SkipList().insertAfter(null, 'a', 'aa').insertAfter('a', 'b', 'bb').insertAfter('b', 'c', 'cc')
  ...
  describe('iterators', () => {
    it('should iterate over values by default', async () => {
      const {SkipList} = await lib;
      let s = new SkipList().insertAfter(null, 'key1', 'value1').insertAfter('key1', 'key2', 'value2')
      assert.deepEqual([...s], ['value1', 'value2'])
    })

    it('should support iterating over keys', async () => {
      const {SkipList} = await lib;
      let s = new SkipList().insertAfter(null, 'key1', 'value1').insertAfter('key1', 'key2', 'value2')
      assert.deepEqual([...s.iterator('keys')], ['key1', 'key2'])
    })

    it('should support iterating over entries', async () => {
      const {SkipList} = await lib;
      let s = new SkipList().insertAfter(null, 'key1', 'value1').insertAfter('key1', 'key2', 'value2')
  ...
  describe('property-based tests', () => {
    ...
    it('should behave like a JS array', () => {
      jsc.assert(jsc.forall(jsc.bless({generator: makeSkipListOps}), async function (ops) {
        const {SkipList} = await lib;
